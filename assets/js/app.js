(() => {
    ...
    document.addEventListener("DOMContentLoaded", function(event) {
        ...
        if (el.textContent.trim() === "Plans") {
            el.setAttribute('data-goto', '#plans');
        }
        });
        const menuItems = document.querySelectorAll(".menu__item");
        menuItems[0].classList.add("menu__item--active");
        // menuItems[5].setAttribute('data-goto', '#plans');
    ...
    function updateContent() {
        ...
        if (width < 550) contentDiv.innerHTML = "Subscribe to our newsletter"; else contentDiv.innerHTML = "Would you like to recieve our Newsletter with offers and upcoming services?";
    }
    updateContent();
    ...
    function showPricing(period) {
        ...
        if (period === "monthly") monthlyElements.forEach((el => el.classList.remove("hidden"))); else if (period === "quarterly") quarterlyElements.forEach((el => el.classList.remove("hidden"))); else if (period === "annual") annualElements.forEach((el => el.classList.remove("hidden")));
        if (!radioMonthly.checked) priceOld.forEach((el => el.classList.remove("card__price-old--hidden")));
    }
    showPricing("monthly");
...
document.addEventListener('DOMContentLoaded', function() {
    const elementToMove = document.querySelector('.footer__button');
    const newParent = document.querySelector('.footer__inputs');
